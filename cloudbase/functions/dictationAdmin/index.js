...
exports.main = async (event = {}) => {
  const action = normalizePrefix(event.action) || 'listAgentDictationRequests';
  ...
  if (action === 'whoami') {
    ...
    const allowList = new Set(adminState.openIds);
    const items = await dictationRuntime.listRequests();
    ...
    return {
      ...
      dictationRequestCount: items.length,
      pendingDictationRequestCount: items.filter((item) => item.status === 'pending').length
    };
  }

  await assertAdmin(event);

  if (action === 'listAgentDictationRequests') {
    const items = await dictationRuntime.listRequests();
    return {
      ok: true,
      items,
  ...
  if (action === 'getAgentDictationRequestStatus') {
    return {
      ok: true,
      request: await dictationRuntime.getRequestStatus(event.requestId || event.id)
    };
  }
  ...
  if (action === 'getAgentDictationRequestStatuses') {
    return {
      ok: true,
      requests: await dictationRuntime.getRequestStatuses(event.requestIds || event.ids)
    };
  }
